...
/// The nine package-level `error` variables of the Go implementation.
#[test]
fn messages_match_go() {
    assert_eq!(
        Error::InvalidBitsTime.to_string(),
        "bit length for time must be 32 or more"
    );
    assert_eq!(
        Error::InvalidBitsSequence.to_string(),
        "invalid bit length for sequence number"
    );
    assert_eq!(
        Error::InvalidBitsMachineId.to_string(),
        "invalid bit length for machine id"
    );
    assert_eq!(Error::InvalidTimeUnit.to_string(), "invalid time unit");
    assert_eq!(
        Error::InvalidSequence.to_string(),
        "invalid sequence number"
    );
    assert_eq!(Error::InvalidMachineId.to_string(), "invalid machine id");
    assert_eq!(Error::StartTimeAhead.to_string(), "start time is ahead");
...
fn sentinels_compare_by_variant() {
    ...
    assert_eq!(Error::InvalidBitsTime, Error::InvalidBitsTime);
    assert_ne!(Error::StartTimeAhead, Error::OverTimeLimit);
    assert_ne!(Error::InvalidMachineId, Error::InvalidSequence);
    assert_ne!(Error::InvalidBitsSequence, Error::InvalidBitsMachineId);
}
