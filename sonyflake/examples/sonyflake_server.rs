//! Port of `example/sonyflake_server.go`.
//!
...
use sonyflake::{awsutil, Settings, Sonyflake};
...
static SF: LazyLock<Sonyflake> = LazyLock::new(|| {
    ...
    match Sonyflake::new_sonyflake(st) {
        Some(sf) => sf,
...
fn handler(stream: &mut TcpStream) -> std::io::Result<()> {
    ...
    let body = to_json(&sonyflake::decompose(id));
...
fn to_json(parts: &BTreeMap<String, u64>) -> String {
    let body = parts
        .iter()
        .map(|(k, v)| format!("\"{k}\":{v}"))
        .collect::<Vec<_>>()
